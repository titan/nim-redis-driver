...
#+begin_src nim :tangle ${BUILDDIR}/redis.nim
  import asyncdispatch, asyncnet, deques, net, options, sequtils, strtabs, strutils, tables
  import redis/lexer
  import redis/syntax
...
#+begin_src nim :noweb-ref lexer-context
  LexerContext = ref object of LexerModel
    input: char
    num: string
    str: string
    strlen: int
...
#+begin_src nim :noweb-ref lexer-action
  proc feed_event[T](ctx: T, sttype: SyntaxTerminalType, val: SyntaxValue) =
    ctx.sctx.input = val
    ctx.sctx.input_type = sttype
    case sttype:
      of stRedisValue: ctx.sctx = redisvalue(ctx.sfsm, ctx.sctx)
      of stRedisString: ctx.sctx = redisstring(ctx.sfsm, ctx.sctx)
      of stRedisError: ctx.sctx = rediserror(ctx.sfsm, ctx.sctx)
      of stRedisInt: ctx.sctx = redisint(ctx.sfsm, ctx.sctx)
      of stRedisNullBulkString: ctx.sctx = redisnullbulkstring(ctx.sfsm, ctx.sctx)
      of stRedisEmptyBulkString: ctx.sctx = redisemptybulkstring(ctx.sfsm, ctx.sctx)
      of stRedisBulkString: ctx.sctx = redisbulkstring(ctx.sfsm, ctx.sctx)
      of stRedisNullArray: ctx.sctx = redisnullarray(ctx.sfsm, ctx.sctx)
      of stRedisArray: ctx.sctx = redisarray(ctx.sfsm, ctx.sctx)
      of stPlus: ctx.sctx = literal_token(ctx.sfsm, ctx.sctx, "+")
      of stString: ctx.sctx = my_string(ctx.sfsm, ctx.sctx)
      of stCrLf: ctx.sctx = literal_token(ctx.sfsm, ctx.sctx, "\r\n")
      of stMinus: ctx.sctx = literal_token(ctx.sfsm, ctx.sctx, "-")
      of stColon: ctx.sctx = literal_token(ctx.sfsm, ctx.sctx, ":")
      of stNumber: ctx.sctx = number(ctx.sfsm, ctx.sctx)
      of stDollar: ctx.sctx = literal_token(ctx.sfsm, ctx.sctx, "$")
      of stZero: ctx.sctx = literal_token(ctx.sfsm, ctx.sctx, "0")
      of stNegative1: ctx.sctx = literal_token(ctx.sfsm, ctx.sctx, "-1")
      of stAsterisk: ctx.sctx = literal_token(ctx.sfsm, ctx.sctx, "*")
      of stRedisArrayItems: ctx.sctx = redisarrayitems(ctx.sfsm, ctx.sctx)

  proc consume_queue[T](ctx: T) =
  ...
  proc output_plus[T](ctx: T): T =
    ...
    ctx0.sctx = literal_token(ctx0.sfsm, ctx0.sctx, "+")
    consume_queue(ctx0)
    result = ctx0
  ...
  proc output_number[T](ctx: T): T =
    ...
    if num == 0:
      ...
      ctx0.sctx = literal_token(ctx0.sfsm, ctx0.sctx, "0")
      ctx0.num = ""
      consume_queue(ctx0)
      result = ctx0
    elif num == -1:
      ...
      ctx0.sctx.input = SyntaxValue(kind: skNumber, num: num)
      ctx0.sctx = literal_token(ctx0.sfsm, ctx0.sctx, "-1")
      ctx0.num = ""
      consume_queue(ctx0)
      result = ctx0
    else:
      ...
      ctx0.sctx.input = SyntaxValue(kind: skNumber, num: num)
      ctx0.sctx = number(ctx0.sfsm, ctx0.sctx)
      ctx0.num = ""
      consume_queue(ctx0)
      result = ctx0
  ...
  proc output_string[T](ctx: T): T =
    ...
    ctx0.sctx = my_string(ctx0.sfsm, ctx0.sctx)
    ctx0.str = ""
    consume_queue(ctx0)
    result = ctx0
...
#+begin_src nim :noweb-ref syntax-context
  ...
  SyntaxContext = ref object of SyntaxModel
    target: SyntaxValue
    input: SyntaxValue
...
#+begin_src nim :noweb-ref syntax-action
  proc shift[T](ctx: T): T =
    var ctx0 = ctx
    ctx0.state_stack.add(ctx0.state)
  ...
  proc reduce_1_to_target[T](ctx: T): T =
    ...
    ctx0.state = ctx0.state_stack.pop
    ctx0.target = value
    return ctx0
    ...
    if not this.pipeline.enabled:
      ...
      when this is Redis:
        ...
        while lctx.sctx.done == false:
          ...
          <<execute-fsm>>
        lctx = eof[LexerContext](lfsm, lctx)
...
#+begin_src nim :noweb-ref init-fsm
  let sfsm = newSyntaxStateMachine[SyntaxContext](this.syntax_action_delegate, this.syntax_guard_delegate)
  ...
  lctx.sfsm = sfsm
  lctx.sctx = sctx
  let lfsm = newLexerStateMachine[LexerContext](this.lexer_action_delegate, this.lexer_guard_delegate)
#+end_src
*** 执行状态机
#+begin_src nim :noweb-ref execute-fsm
  for ch in response:
    lctx.input = ch
    lctx = input[LexerContext](lfsm, lctx, ch)
#+end_src
** key/value
*** 框架
...
#+end_src
*** flushall
#+begin_src nim :noweb-ref flushall
  proc flushall*(redis: Redis | AsyncRedis): Future[string] {.multisync.} =
    ...
    when redis is Redis:
      ...
      await redis.send("FLUSHALL", "ASYNC")
    let resp = await redis.recv
    result = resp.toString.get
#+end_src
*** flushdb
#+begin_src nim :noweb-ref flushdb
  proc flushdb*(redis: Redis | AsyncRedis): Future[string] {.multisync.} =
    ## Delete all the keys of the currently selected DB. This command
    ## never fails.
    when redis is Redis:
      await redis.send("FLUSHDB")
    else:
      await redis.send("FLUSHDB", "ASYNC")
    let resp = await redis.recv
    result = resp.toString.get
#+end_src
*** incr
...
#+end_src
*** select
#+begin_src nim :noweb-ref select
  proc select*(redis: Redis | AsyncRedis, index: int): Future[string] {.multisync.} =
    ## Select the Redis logical database having the specified zero-based
    ## numeric index. New connections always use the database 0.
    await redis.send("SELECT", $index)
    let resp = await redis.recv
...
#+begin_src nim :noweb-ref xclaim
  proc xclaim*(redis: Redis | AsyncRedis, key: string, group: string, consumer: string, minidletime: uint32, ids: seq[string], idle: uint32 = 0, time: uint32 = 0, retrycount: uint32 = 0, force: bool = false): Future[seq[(string, StringTableRef)]] {.multisync.} =
...
#+begin_src nim :noweb-ref xrange
  proc xrange*(redis: Redis | AsyncRedis, key: string, start: string = "-", stop: string = "+", count: uint32 = 0): Future[seq[(string, StringTableRef)]] {.multisync.} =
...
#+begin_src nim :noweb-ref xread
  proc xread*(redis: Redis | AsyncRedis, keyids: seq[(string, string)], count: uint32 = 0, timeout: uint32 = 0): Future[seq[(string, seq[(string, StringTableRef)])]] {.multisync.} =
...
#+begin_src nim :noweb-ref xreadgroup
  proc xreadgroup*(redis: Redis | AsyncRedis, group: string, consumer: string, keyids: seq[(string, string)], count: uint32 = 0, timeout: uint32 = 0, noack: bool = false): Future[seq[(string, seq[(string, StringTableRef)])]] {.multisync.} =
...
#+begin_src nim :noweb-ref xrevrange
  proc xrevrange*(redis: Redis | AsyncRedis, key: string, stop: string = "+", start: string = "-", count: uint32 = 0): Future[seq[(string, StringTableRef)]] {.multisync.} =
...
#+begin_src nim :noweb-ref converter
  ...
  proc toTable(v: RedisValue): StringTableRef =
  ...
  proc toSequenceTupleTable(v: RedisValue): seq[(string, StringTableRef)] =
    result = @[]
