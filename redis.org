  ...
  proc toTable(v: RedisValue): TableRef[string, string] =
    ...
    if v.kind == rkArray:
      if v.array.isSome:
        ...
        for i in 0..int(len(array)/2):
          let field = if array[i].kind == rkBulkString and array[i].bulkstr.isSome: array[i].bulkstr.get() else: ""
          let value = if array[i + 1].kind == rkBulkString and array[i + 1].bulkstr.isSome: array[i + 1].bulkstr.get() else: ""
          result[field] = value
